...
- `context` -> always escalate. A worker whose context reached the warn level or compacted may be running a task too large to finish well, and only the supervisor can split it.
