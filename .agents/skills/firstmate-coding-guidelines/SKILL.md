...
The model to copy is `AGENTS.md` section 8's "Away-mode stub": it keeps only the load trigger, the ownership-transfer rule, and the exit condition inline, and points everything else, including the injection marker format, at the `/afk` skill.
