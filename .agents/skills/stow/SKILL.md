---
name: stow
description: Sweep the current session for uncaptured durable knowledge, route durable knowledge to its authoritative owner, and curate the home's tiered, decaying startup memory before a context reset. Use when the captain invokes /stow, whenever durable knowledge needs routing, before a session reset or context compaction, or periodically to keep operational memory current.
user-invocable: true
metadata:
  internal: true
---
...
2. **Route each finding to its most specific owner.**
   ...
   - Knowledge useful to almost every contributor to one project belongs in that project's committed `AGENTS.md` through its selected delivery path and `bin/fm-ensure-agents-md.sh`, never through a direct firstmate write.
   - Knowledge general to every Firstmate user belongs in this repo's shared tracked surface through its normal delivery path.
   Keep fleet delivery posture and captain-private strategy out of project memory.
3. **Write within the existing boundaries.**
   - Captain preferences and fleet-local operational facts belong in the destination selected by the map above after the required whole-file curation pass.
...
4. **Use inspect-then-update.**
   ...
   The only graduation moves are promotion to tracked shared material through a PR, folding a learning into the captain-preference destination selected above, archiving a stale entry to `data/memory-archive.md`, autonomous offload of an eligible non-pinned conditional entry to an already-existing allowed owner through the reduce flow above, captain-approved offload of a pinned durable conditional entry to a JIT-loaded owner executed through the migration step above, or deletion of an entry that is a duplicate or already preserved through a stronger existing owner.
...
Extend the completion receipt with one entry per secondmate alongside the primary's own, carrying whether inherited shared material was `pushed`, `unchanged`, `skipped`, or `error`, that home's budget before and after, its per-file actions, its exceptions, and whether that home swept itself or was curated from here.
