...
const ARM_READY_TIMEOUT_DEFAULT_MS = process.platform === "win32" ? 35000 : 50000;
...
function classifyArmClose(stdout, stderr, code, signal) {
  ...
  const reason = combined.split(/\r?\n/).find((line) => /^(signal:|stale:|check:|context:|heartbeat($|:))/.test(line));
...
function observeArmOutput(stdout, stderr, settleReadiness) {
  const combined = `${stdout}\n${stderr}`;
  if (combined.split(/\r?\n/).some((line) => /^(signal:|stale:|check:|context:|heartbeat($|:))/.test(line))) {
    setArmStatus("wake");
