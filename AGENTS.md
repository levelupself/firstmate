...
.env                 optional Relay pairing token and optional LINEAR_API_KEY; LOCAL, gitignored; presence-gates section 14 and the Linear mirror (docs/linear.md)
...
config/cockpit-layout  Herdr cockpit fleet-region direction, order, and ratio; LOCAL, gitignored, and not inherited; absent means the default full-width 28% band above the supervisor; see docs/configuration.md "Cockpit fleet layout"
config/cockpit-sections  Herdr cockpit fleet-region pane arrangement, one comma-separated section group per line; LOCAL, gitignored, and not inherited; absent means the default decisions, ready, then in-flight and blocked panes; see docs/configuration.md "Cockpit fleet sections"
config/startup-memory-budget     primary-authoritative per-home startup-memory budget; LOCAL, gitignored, materialized as 7,500 estimated tokens by locked primary bootstrap and inherited into secondmate homes; see docs/configuration.md "Startup memory budget"
config/agents-md-budget          primary-authoritative AGENTS.md budget; LOCAL, gitignored, materialized as 25,000 estimated tokens by locked primary bootstrap and inherited into secondmate homes; see docs/configuration.md "AGENTS.md budget"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  cost-attribution.tsv  append-only deterministic lifecycle capture written by teardown before volatile task metadata is removed; an ingestion input for the effort store (docs/effort-store.md)
...
state/               runtime records and signals; gitignored
  ...
  .herdr-cockpit     per-home exact Herdr cockpit frame binding; docs/herdr-backend.md "Watching and task containers"
  .cockpit-focus.lock  single-flight lock for the cockpit viewport's focus reaction; docs/herdr-backend.md "Watching and task containers"
...
2. **Bootstrap** - locked Herdr cockpit adoption runs before the mutating sweeps, and detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
...
A silent bootstrap section needs no action; for any printed actionable diagnostic line, including `MISSING_OPTIONAL:`, load `bootstrap-diagnostics` and follow its owner procedure.
...
For a no-mistakes ship, the worker continues straight from its implementation commit into validation on its own; send the harness invocation owned by `harness-adapters` to that same worker only when it stopped without starting one.
...
Keep a PR-based ship task live until its open PR lands whenever practical; this is advisory because the sanctioned merge path also handles a safely delivered task that was torn down early.
