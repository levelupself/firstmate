...
bin/fm-test-run.sh --changed   # conservative changed-file-informed set (never silent full suite; refuses when it selects nothing)
...
A run that executes no test refuses instead of exiting 0, so "I ran the suite and it passed" can never be true of a run that exercised nothing.
`--aggregate-json` holds the same line across lanes: it refuses a lane timing artifact reporting `summary.total` of 0 rather than summing a phantom zero into the combined summary.
...
Family selection is the ordinary local path; a local `--all` full walk is optional and advisory, including when red, and is not a delivery gate.
...
CI is the delivery gate; check the CI timing artifact for the failing script before treating a local-only failure as a regression.
...
A behavior assertion must pin the property it names rather than a host-dependent proxy for it, because a wall-clock budget, a `ppid` of 1, and a tool resolving on the caller's `PATH` each hold on one machine and silently void or fail the same case on another.
Count the operation the subject actually performs, assert the process relationship the subject actually needs, and publish a fixture's remote-side dependencies where that fixture's own resolution order will find them.
