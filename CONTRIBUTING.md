...
A run that executes no test refuses instead of exiting 0, and `--aggregate-json` holds that same line across lanes, so "I ran the suite and it passed" can never be true of a run that exercised nothing; the header owns the exact per-mode rules.
