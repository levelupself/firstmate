...
An upstream catch-up can instead pass when its commit graph contains exactly one two-parent merge from the repository fork parent's history, contains the current base, and carries no ordinary authored commits outside that merge spine.
A title, body, label, author, or commit message claiming to be a catch-up has no bearing on that exemption.
