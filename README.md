...
- **A visible crew** - every crewmate works in a tmux window, experimental Herdr cockpit pane or standalone tab, Zellij tab, cmux workspace, or Orca terminal you can watch or type into; the first mate reconciles.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   tmux windows, Herdr panes/tabs, Zellij tabs, cmux workspaces, or Orca terminals
...
| `/surface`         | Bring a named URL, directory, or file into view through its appropriate browser, Lavish board, Windows application, File Explorer location, or in-session text excerpt without requiring an application choice |
| `/updatefirstmate` | Fast-forward the running firstmate and its secondmates from origin, report pending changes from an optional upstream template remote without merging them, then re-read instructions and nudge secondmates |
...
- [docs/task-usage.md](docs/task-usage.md) - per-task codeburn attribution, supported coverage, and durable snapshots.
- [docs/effort-store.md](docs/effort-store.md) - automatic lifecycle capture and reporting for the derived agentic-engineering effort store.
