...
#
# Worktree acquisition (leasing a copy, running `treehouse enter` into it
# inside the pane, and polling the pane's cwd) is unchanged by this
# extraction: P1 scopes only the session provider, not the worktree provider,
# so fm-spawn.sh still drives that part inline with these same
...
# (`treehouse enter`, the GOTMPDIR export) that already ran this exact sequence
