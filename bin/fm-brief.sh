...
Include this artifact-style constraint in \`--intent\`: pipeline-authored risk rationales, fix summaries, and PR bodies must use role-neutral prose with no second person and no captain, user, firstmate, or crewmate role nouns; this constraint does not apply to product code, prompts, or user-facing copy.
