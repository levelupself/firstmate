...
# <workspace-root> as its cwd AND identifies as an interactive session. Two
# independent metadata fields carry that second judgement - originator and
# source - so neither one alone is load-bearing. The filter matters because
...
fm_busy_codex_matching_logs() {  # <sessions-root> <workspace-root> [<from-day>]
  ...
  command -v jq >/dev/null 2>&1 || return 1
  {
    ...
    | select(type == "object" and .cwd? == $ws
             and (.originator? == "codex-tui" or .source? == "cli"))
    | $row.file
  '
}
