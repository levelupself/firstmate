...
#     an actionable arm close (signal:/stale:/check:/context:/heartbeat) prints one
...
while [ "$attempt" -lt "$AUTOARM_ATTEMPTS" ]; do
  ...
  ACTIONABLE=0
  if [ -n "$OUT" ]; then
    grep -Eq '^(signal:|stale:|check:|context:|heartbeat($|:))' "$OUT" 2>/dev/null && ACTIONABLE=1
  fi
...
if [ "$ACTIONABLE" -eq 1 ]; then
  write_epoch rewake
  {
    ...
    [ -n "$OUT" ] && grep -E '^(signal:|stale:|check:|context:|heartbeat)' "$OUT" 2>/dev/null | head -8
...
if [ ! -e "$FAILURE_NOTICE" ]; then
  write_epoch failed
  {
    ...
    [ -n "$OUT" ] && grep -E '^(watcher:|signal:|stale:|check:|context:|heartbeat)' "$OUT" 2>/dev/null | head -8
