...
usage_json_for_id() {  # <id> <harness> <collected-dir>
  ...
  if printf '%s' "$raw" | jq -e 'type == "object" and (.schema == "fm-task-usage.v1" or .schema == "fm-task-usage.v2" or .schema == "fm-task-usage.v3")' >/dev/null 2>&1; then
    printf '%s' "$raw"
    return 0
  fi
  if [ -f "$cache" ]; then
    cached=$(cat "$cache" 2>/dev/null || true)
    if printf '%s' "$cached" | jq -e 'type == "object" and (.schema == "fm-task-usage.v1" or .schema == "fm-task-usage.v2" or .schema == "fm-task-usage.v3")' >/dev/null 2>&1; then
...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
      ...
      | ($scalar.remote_host) as $remote_host
      | ($scalar.remote_root) as $remote_root
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      record=$(printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
        ...
        --arg id "$id" --arg home "$home" --arg host "$host" --arg state "$state" --arg observed "$SNAPSHOT_NOW" \
      ...
      record=$(printf '%s\n%s\n%s\n%s\n%s\n%s\n' \
        ...
        --arg id "$id" --arg home "$home" --arg host "$host" --arg observed "$SNAPSHOT_NOW" \
        --argjson remote "$remote" \
