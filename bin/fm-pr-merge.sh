...
# forge mutation and advances that receipt to phase=merged after success.
...
receipt_matches_request() {
  local authorization merged_epoch phase prepared_epoch
...
write_provenance_receipt() {
  local phase=$1 authorization=$2 prepared_epoch=$3 merged_epoch=${4:-} tmp
  ...
  tmp=$(mktemp "$PROVENANCE_DIR/.$ID.receipt.XXXXXX") || return 1
  {
    ...
    [ -z "$merged_epoch" ] || printf 'merged_epoch=%s\n' "$merged_epoch"
