...
if [ "$READ_ONLY" -eq 0 ]; then
  if [ "$REEMIT" -eq 0 ]; then
    ...
    BACKLOG_INTEGRITY_DIAGNOSTIC=$("$SCRIPT_DIR/fm-backlog-integrity.sh" reconcile 2>&1) \
  ...
  FM_BOOTSTRAP_PRIMARY_HARNESS="$PRIMARY_HARNESS" \
...
if [ "$READ_ONLY" -eq 1 ]; then
  BOOT_OUT=$(
    ...
    FM_BOOTSTRAP_DETECT_ONLY=1 FM_BOOTSTRAP_NETWORK=skip \
      FM_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" FM_BOOTSTRAP_PRIMARY_HARNESS="$PRIMARY_HARNESS" \
  ...
  BOOT_OUT=$(
    ...
    FM_BOOTSTRAP_NETWORK=skip FM_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" FM_BOOTSTRAP_PRIMARY_HARNESS="$PRIMARY_HARNESS" \
      "$SCRIPT_DIR/fm-bootstrap.sh" 2>&1
  )
fi
if [ -n "$BOOT_OUT" ]; then
