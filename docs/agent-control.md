...
- The case relaunch cannot serve - a recorded endpoint that is authoritatively gone AND a record naming a different copy than the one holding the work - belongs to `fm-spawn --reattach-worktree`, which is a recovery path rather than a control verb and is not reachable through this plane.
  ...
  For displaced-copy recovery with `fm-spawn --reacquire-worktree`, see the [recovery decision procedure](../.agents/skills/stuck-crewmate-recovery/SKILL.md#session-start-reconciliation-for-a-dead-ordinary-direct-report); the script header owns each path's identity, ownership, preservation, and rollback mechanics.
