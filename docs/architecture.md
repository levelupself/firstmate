...
Cleanup is the deliberate exception to that last rule, because completed-history retention is entitled to prune a finished task's Done entry: requiring the row would leave the worktree, endpoint, and state of already-landed work held forever with no path but hand recovery, and each recovery prunes another entry.
The landed-work guarantee never lived in that row, so cleanup falls back to evidence retention cannot delete - a merged PR containing the local work or content present in the current default branch, or an identity-bound merged-PR or local-landing receipt or completed scout report - and refuses when neither exists, because absence of a record is never permission to discard.
Pinning retention to runtime state was rejected as the weaker design: it would make an external queue's retention policy safety-critical and would only postpone the same loss through archiving, a hand edit, or a home migration.
Reconciliation reports every task whose durable runtime record outlives its backlog row as `stranded=<id>`, so state left holding a worktree and endpoint is visible rather than silent.
