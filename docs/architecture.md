...
The helper advances that receipt to merged with the forge merge time when available, the forge default branch, and the merge commit only after the forge reports the PR merged and an ahead-or-identical forge comparison proves that commit is on the default branch.
