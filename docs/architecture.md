...
When the project's `origin` is a local filesystem mirror, that confirmed commit is fetched through the configured GitHub remote and pushed to the mirror by fast-forward only before the merged outcome is recorded; divergence leaves the receipt prepared, preserves every mirror commit, and is reported as a post-forge refusal.
