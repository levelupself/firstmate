...
Because every such cleanup is advisory, [`docs/configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) also defines the three deterministic footprint gates - teardown's return post-condition, spawn's pool-acquisition pre-condition, and the scheduled sweep's pool budget wake - that share one measurement in [`bin/fm-pool-footprint-lib.sh`](../bin/fm-pool-footprint-lib.sh) and one budget pair.
