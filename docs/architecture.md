...
The helper advances that receipt to merged with the forge merge time when available, the forge default branch, and the merge commit only after an ahead-or-identical forge comparison proves that commit is on the default branch; post-merge confirmation is retried briefly and an unreadable or genuinely unconfirmed result refuses the merge outcome with the receipt still prepared.
