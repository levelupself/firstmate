...
A `context:` wake is the watcher's read of a live ordinary task's own session record, never its pane: `bin/fm-context-watch.mjs tick` folds only the bytes appended since the last poll into the task's context size, peak, and compaction count under `state/<id>.context-watch`, and the watcher enqueues `context: <id> <N>k tokens (warn <W>k)` the first time the size reaches `FM_CONTEXT_WARN_TOKENS` and at each further `FM_CONTEXT_WARN_STEP`, plus `context: <id> compacted (n=<count>)` on each new compaction, recording the surfaced level in `state/.context-surfaced-<id>` after the enqueue so an unchanged record is absorbed.
A task past the warn level is probably too large for one worker and is a candidate for splitting; `bin/fm-context-watch.sh <id>` prints the same reading on demand, and the same fold is what `bin/fm-model-bench-analyze.mjs` reports for a bench arm.
