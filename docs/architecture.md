...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records live-task `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` and writes a durable `data/pr-merges/<task-id>.receipt` before calling `gh-axi pr merge`.
