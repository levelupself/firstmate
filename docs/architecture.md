...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, context wakes, and heartbeat backstop hits.
A `context:` wake comes from a live ordinary task's stamped session record, never its pane; [`bin/fm-context-watch.mjs`](../bin/fm-context-watch.mjs) owns incremental reads, binding, and surfaced-marker mechanics, with thresholds configured in [`configuration.md`](configuration.md).
The watcher enqueues before recording suppression, so a failed enqueue remains eligible on the next poll.
The shared fold in `bin/fm-model-bench-analyze.mjs` also supplies the bench-arm context signal; handling guidance lives in [`supervision-protocols/common.md`](supervision-protocols/common.md).
...
A busy worker whose latest `working:` line names the file its long foreground job keeps writing, with the whole-word `long-run=<absolute path>` token owned by `bin/fm-classify-lib.sh`, stays off that escalation only while the named file was modified within `FM_STALE_ESCALATE_SECS`; the declaration points at evidence rather than asserting liveness, so a missing or quiet artifact escalates exactly as an undeclared pane does.
When a declaration gives no relief, the watcher's triage log names the declared artifact and whether it is missing or how long it has been quiet, so a mis-declared worker can be told from an undeclared one.
...
`bin/fm-pr-landed-lib.sh` owns the default-branch landing evidence shared by merge confirmation and PR polling.
...
A crew parked on a captain decision (`captain-held:`) gets the same cadence, and so does either declaration whether or not the backend still reports the crew's agent alive: a park declared by a crew that is still running is the ordinary shape of a decision hold, so liveness makes the park credible rather than invalidating it.
The suppression stays bounded because the declaration itself already surfaced through the no-verb signal path, the recheck cadence is anchored on the park's own status-file age, and any later non-park status line drops the crew back into ordinary wedge escalation; only an actively running pipeline outranks the declaration, and the secondmate idle-endpoint exemption is unchanged.
...
Answerer-closes stops at the reserved decision-key namespaces: a reserved key (today the `pending-reply-` namespace) is opened and closed only by `bin/fm-pending-reply-lib.sh`, so `fm-send` refuses `--resolve-key` for one before anything is sent and the OPEN DECISIONS section hints that library's close flow instead of the answerer-closes command for those entries, while `bin/fm-classify-lib.sh`'s `fm_decision_key_is_reserved` is the sole owner of that namespace test for the fold, the unread surface, and both CLIs.
This home's answerer close, pending-reply escalation close, and captain-held parking bookkeeping use the provenance-guarded append owned by `bin/fm-wake-lib.sh`, so they advance the watcher marker only across their own bytes when all earlier bytes were already announced; pending or interleaved foreign bytes fail toward an ordinary wake.
...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current lifecycle state, endpoint probes, PR/report pointers, task reports, per-task codeburn usage (docs/task-usage.md), the per-task context signal from the watcher's cache, bounded current summaries from registered homes, and return-channel guidance.
...
The existing turn-end guard remains the final backstop for every harness-engine protocol, with pi-signed sharing Pi's protocol, the `--claude` mode cooperating with the auto-arm's recorded recovery ownership, and Cursor's `--cursor` mode rendering a block as one bounded follow-up because its `stop` step cannot be blocked.
...
Each converted adapter reports its own turn lifecycle through a machine-readable contract the vendor already exposes, rather than through rendered footer text: Pi and pi-signed through the Firstmate-owned extension's `agent_start` and `agent_settled` confirmed by `ctx.isIdle()`, OpenCode through its plugin's semantic `session.status`, Claude through owned `UserPromptSubmit`, `Stop`, `StopFailure`, and `SessionEnd` hooks, Muse through its session log, Cursor through its conversation transcript, and Codex through its per-session rollout log.
Kimi behind Pi inherits Pi's lifecycle.
Standalone Kimi classifies unknown behind explicit probes until a semantic source is live-verified for it, and Grok keeps one clearly isolated rendered-tail fallback that can only ever classify a Grok task.
...
It owns task endpoint creation, bounded capture, text/key sends, current-path reads for the spawn-time worktree-discovery settle wait when the backend does not create the worktree itself, live-window fallback lookup, agent-process liveness probes where verified, and endpoint teardown.
...
Herdr places tasks in exact recorded panes: an adopted native-supervisor cockpit splits ordinary children into its persistent per-home viewport, while the fallback topology uses one tab per task; [Watching and task containers](herdr-backend.md#watching-and-task-containers) owns frame identity, placement, degradation, and recovery scope.
...
Where a pane enters a pooled worktree, that resolved path is the pane shell's own `pwd -P` rather than the terminal backend's reported pane path, because a reported path is a rendered surface that can name a real pooled worktree the shell never entered.
Confirming it and launching are therefore one step: a shell still in the primary checkout launches no agent and records no `worktree=` at all, and a shell that is somewhere else is recorded where it actually is.
...
[`docs/configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) also defines the Treehouse footprint gates - teardown's return post-condition, spawn's pool-acquisition pre-condition, and the scheduled sweep's pool budget wake - that share one measurement in [`bin/fm-pool-footprint-lib.sh`](../bin/fm-pool-footprint-lib.sh) and one budget pair.
...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-control.sh`, `fm-teardown.sh`, and `fm-endpoint-bind-migrate.sh` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records live-task `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` and writes a durable prepared `data/pr-merges/<task-id>.receipt` before calling `gh-axi pr merge`.
Before that forge mutation, `bin/fm-pr-checks.sh` reports CI as `passing`, `failing`, or `ABSENT` alongside the forge's `mergeable_state`, and the merge path refuses both `failing` and `ABSENT` while preserving the prepared receipt.
The helper advances that receipt to merged with the forge merge time when available, the forge default branch, and the merge commit only after the shared landing-evidence test in [`bin/fm-pr-landed-lib.sh`](../bin/fm-pr-landed-lib.sh) succeeds.
[`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the post-mutation confirmation retry budget, deadline semantics, and verification required after failed confirmation.
When the project's `origin` is a local filesystem mirror, that confirmed commit is fetched through the configured GitHub remote and the verified bare mirror ref is updated directly and atomically with its observed old value before the merged outcome is recorded; divergence or a concurrent ref move leaves the receipt prepared, preserves every mirror commit, and is reported as a post-forge refusal.
...
That Done-history authorization carries no launch identity, so [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns its receipt shape, its project-registry checkout resolution, and the launch-bound lifecycle capture it skips.
...
Lifecycle refusal is asymmetric: dispatch refuses an absent backlog because no work has started, while a completed merge or safe cleanup proceeds and explicitly reports an absent backlog because refusal would strand landed work; dispatch and lifecycle writes still refuse when an existing backlog lacks the task row and its Done history does not record the task.
Cleanup is the deliberate exception to that last rule, because completed-history retention is entitled to prune a finished task's Done entry: requiring the row would strand the worktree, endpoint, and state of already-landed work.
The landing write is the other exception: a task closed as Done before its PR merged can have that row pruned into `data/done-archive.md` by the time the merge records its outcome, and the archived Done row already carries the outcome the write would record, so the merge preflight accepts it for that exact task and PR, `landed` accepts a task its Done history records, and every other absent row keeps the refusal.
The landed-work guarantee never lived in that row; [`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the independent evidence requirements for cleanup after retention.
Pinning retention to runtime state was rejected as the weaker design: the external queue's `done_keep` policy cannot condition pruning on runtime state, would become safety-critical, and would only postpone the same loss through archiving, a hand edit, or a home migration.
Accepting the done-archive row as cleanup's landing proof was also rejected because it moves the record-existence dependency to another file while retaining unverified text as authority; the merge path reads that same row only as the task's recorded outcome, never as proof that work landed.
Independent landing proof is stronger than a Done row: current work can be checked directly, while receipts preserve launch identity and re-verifiable delivery evidence after the worktree is gone.
Reconciliation makes surviving task records visible through the stranded-record diagnostic owned by [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md).
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, and the copy-binding check that keeps a rerun from touching a pool copy since leased to another task; the pool's own binding is the durable per-task lease [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) takes under the task id and only a successful teardown return releases, with [`docs/configuration.md`](configuration.md) "Runtime backend" as the operator-facing owner.
...
Upstream movement inventories changes for deliberate review and selection without making a wholesale merge due.
Each wanted change enters through the normal delivery path after its dependencies and interaction with fork-only behavior are reviewed, never through a reset, rebase, force-push, or prefer-theirs conflict pass.
When the selected delivery method merges upstream history, it uses a separate reviewed merge commit.
