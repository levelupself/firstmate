...
Those cleanups are advisory by construction, so three hard footprint gates make build-output hygiene deterministic at the chokepoints every copy passes through.
A copy's ignored footprint is every path `git ls-files --others --ignored --exclude-standard --directory -z` reports, summed with `du` without following symlinks; `bin/fm-pool-footprint.sh measure <copy>` and `report <copy>` expose that measurement, and `bin/fm-pool-footprint-lib.sh` owns it.
...
An unreadable project inventory remains an explicit audit failure and wakes once while unchanged, while failures cleaning individual copies do not omit other captured copies from the audit.
