...
FM_CLAUDE_AUTOARM_EPOCH_FRESH=15   # seconds a recorded auto-arm outcome remains eligible for the current event epoch's recovery or failure decision; a rewake epoch naming the current session is instead honored once per epoch regardless of age, because the recovery turn it created has no bounded length
