...
Bootstrap also detects three optional enhancements without blocking a healthy session: codeburn provides automatic task-usage snapshots, `@infisical/cli` provides Infisical credential workflows when a project needs that boundary, and Herdr enables later Herdr-backed dispatch when it is not already the resolved backend.
