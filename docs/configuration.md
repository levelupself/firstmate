...
Because a task torn down early with its PR still open keeps its record for the merge poll while its copy goes back to the pool, `bin/fm-teardown.sh` proves a Treehouse copy is still the task's own before it reads, prunes, resets, kills inside, or returns it, keyed on the per-project worktree allocation ledger under `data/worktree-allocations/` (acquire events written by `bin/fm-spawn.sh`, release events by teardown after a successful return, queried with `bin/fm-worktree-allocation.sh holder`), other live records in this home, the copy's checked-out branch, and the pool's own `treehouse status --json` inventory.
When that task's record already carries `teardown_at=` and the ledger names another task as the copy's holder, teardown prints `copy already returned; held by <task-id>`, retires only its own records and recorded endpoint, and exits 0; a copy another task holds otherwise refuses by name, and `--force` does not override that refusal because the work is not the task's to discard.
