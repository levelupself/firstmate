...
A copy's ignored footprint is every path `git ls-files --others --ignored --exclude-standard --directory` reports, summed with `du` without following symlinks; `bin/fm-pool-footprint.sh measure <copy>` and `report <copy>` expose that measurement, and `bin/fm-pool-footprint-lib.sh` owns it.
...
After each scheduled hourly sweep, `bin/fm-pool-footprint.sh --pool-audit` sums the footprint per project pool; when a pool exceeds `FM_POOL_TOTAL_BUDGET_GB`, it writes `state/pool-footprint.over-budget` and registers `state/pool-footprint.check.sh` through `bin/fm-check-register.sh`, so the watcher wakes firstmate exactly once per distinct over-budget total, naming the pool total and its three largest copies, and stays silent while that total is unchanged.
