...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, task-to-PR merge receipts under `data/pr-merges/`, and per-PR CI run ledgers under `data/pr-ci/`.
...
The helper carries each moved item's queued decision holds with it so unresolved decisions do not remain in the originating home after their work moves.
...
The pool's own binding for a task's copy is a durable per-task lease: `bin/fm-spawn.sh` acquires every pooled copy with `treehouse get --lease --lease-holder <task-id>`, records the lease identity as `pool_lease_id=` in `state/<id>.meta`, and the pool never hands that copy to a later acquisition or prunes it, with or without a process inside, until `bin/fm-teardown.sh` returns it under that exact lease after a successful landed-work test.
The pool remembered an interactive `treehouse get` only by the acquiring shell's process, so before leases a parked or finished task's clean copy read as available again once no process had its working directory inside it, including after a host reboot, while this home's `state/<id>.meta` still bound it; a record spawned before leases carries no `pool_lease_id=` and keeps that pre-lease behavior.
A task record binds its `worktree=` for this home's own spawns until `bin/fm-teardown.sh` stamps `teardown_at=` into it after the landed-work test or removes it; a status log or report alone binds nothing.
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns the lease acquisition, the bound-copy guard that still steers around a pre-lease copy the pool would hand out, empty versus unreadable inventory handling, the final ownership check, and the lease's release when a spawn fails before its record binds the copy; [`tests/fm-pool-lease.test.sh`](../tests/fm-pool-lease.test.sh) and [`tests/fm-spawn-pool-slot-binding.test.sh`](../tests/fm-spawn-pool-slot-binding.test.sh) cover that contract.
A task whose recorded copy was already taken (a pre-lease copy the pool handed out, or a copy returned by an early cleanup) is recovered with `bin/fm-spawn.sh <id> --reacquire-worktree`, which leases a fresh copy holding the task's branch at its current head on the task's recorded tmux or herdr backend; `bin/fm-spawn.sh --help` owns both mechanics and [`stuck-crewmate-recovery`](../.agents/skills/stuck-crewmate-recovery/SKILL.md) owns when to use which recovery.
Because a task torn down early with its PR still open keeps its record for the merge poll while its copy goes back to the pool, `bin/fm-teardown.sh` proves a Treehouse copy is still the task's own before it reads, prunes, resets, kills inside, or returns it: for a lease-bound record the pool's own `treehouse status --json` inventory must still hold the recorded lease, and the return is pinned with `--if-lease-id` so the pool itself refuses to reset a copy it has since released or re-leased; every record is additionally keyed on the per-project worktree allocation ledger under `data/worktree-allocations/` (acquire events written by `bin/fm-spawn.sh`, release events by teardown after a successful return, queried with `bin/fm-worktree-allocation.sh holder`), other live records in this home, the copy's checked-out branch, and, for a pre-lease record, the inventory's occupant.
When that task's record already carries `teardown_at=` and the pool no longer holds its lease or the ledger names another task as the copy's holder, teardown prints `copy already returned; held by <task-id> ...` or `copy already returned; its pool lease <id> was released ...`, retires only its own records and recorded endpoint, and exits 0; a live record whose lease the pool no longer holds is a lease inconsistency refused by name, a copy another task holds otherwise refuses by name, and `--force` does not override those refusals because the work is not the task's to discard.
The [`fm-teardown.sh` header](../bin/fm-teardown.sh) owns the copy-binding check and the [`fm-worktree-allocation.sh` header](../bin/fm-worktree-allocation.sh) owns the ledger; [`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh) and [`tests/fm-worktree-allocation.test.sh`](../tests/fm-worktree-allocation.test.sh) cover them.
...
Treehouse pool hygiene also has three footprint gates: teardown return, spawn acquisition, and a scheduled pool budget alert.
A copy's ignored footprint is every path `git ls-files --others --ignored --exclude-standard --directory -z` reports, summed as allocated disk usage with `du -sk` without following symlinks; `bin/fm-pool-footprint.sh measure <copy>` and `report <copy>` expose that measurement, and `bin/fm-pool-footprint-lib.sh` owns it.
`FM_POOL_COPY_BUDGET_GB` (default 12 decimal GB) bounds one copy and `FM_POOL_TOTAL_BUDGET_GB` (default 80 decimal GB) bounds one project's whole pool; both must be positive decimal values, and an invalid value stops the gate rather than defaulting.
At teardown, once the landed-work, run-abort, and process-reap checks have passed, the copy is pruned through the return rule table and then measured; a copy still over `FM_POOL_COPY_BUDGET_GB` is neither returned to the pool nor retired, `bin/fm-teardown.sh` prints `REFUSED: copy over footprint budget` with the ten largest ignored paths and exits non-zero before stamping `teardown_at=`, and `--footprint-override --reason <text>` returns it anyway while appending the reason to `state/teardown.log`.
The same teardown gate applies to child copies during forced retirement, including nested homes, and a refusal preserves their records.
`--force` does not bypass the footprint gate, and a footprint override cannot bypass a failed prune, measurement, or override-log write; the teardown header owns argument validation and Git-lock handling.
At spawn, every free unbound copy the pool inventory reports is measured before any copy is entered; an over-budget copy is pruned and re-measured, one still over budget is never handed to the new worker, and the spawn steers into a copy that fits or refuses naming the copies that do not.
An unmeasurable free copy is refused; the spawn header owns the unreadable-inventory compatibility fallback.
After each scheduled hourly sweep, `bin/fm-pool-footprint.sh --pool-audit` sums the footprint per project pool; when a pool exceeds `FM_POOL_TOTAL_BUDGET_GB`, it writes `state/pool-footprint.over-budget` and registers `state/pool-footprint.check.sh` through `bin/fm-check-register.sh`, so the watcher wakes firstmate exactly once per distinct exact byte total for each canonical project identity, naming the pool total and its three largest copies, and stays silent while that total is unchanged.
...
The local `config/herdr-presentation-spaces` file opts a home out of, or explicitly in to, Herdr's default-on disposable single-task visual projection when no adopted cockpit frame owns placement; [Presentation spaces](herdr-backend.md#presentation-spaces) owns its accepted values, default, Herdr version floor, migration, behavior, safety limits, recovery contract, and narrow locked session-start cleanup of exact restored idle-shell children.
...
Claude operators who need a machine-local projects directory set `FM_PROJECTS_OVERRIDE` in `.claude/settings.local.json`, whose `env` object is merged with the tracked project settings and remains ignored by Git.
For example, use `{"env":{"FM_PROJECTS_OVERRIDE":"/path/to/projects"}}`, then start a new Claude session and verify the resolved value with `printf '%s\n' "$FM_PROJECTS_OVERRIDE"` before relying on it.
...
The [`harness-adapters` entry point](../.agents/skills/harness-adapters/SKILL.md) owns the verified set and routes to the adapter-detail skill holding each harness's busy-state source, interrupt and exit behavior, skill-invocation syntax, and per-harness quirks.
...
The universal required toolchain is node, git, gh with GitHub auth via `gh auth login`, pnpm, ripgrep, pinned ShellCheck 0.11.0, no-mistakes v1.31.2 or newer, compatible gh-axi, chrome-devtools-axi, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
In that list, pnpm supports project package workflows, ripgrep is the repository-search tool required by the operating instructions, and ShellCheck runs the validation lint gate.
The pinned ShellCheck installer extracts a `.tar.xz` archive, so xz-utils is required while ShellCheck is missing or incompatible and is reported as an optional reinstallation dependency once the exact pin is already usable.
No-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and lavish-axi cover GitHub, browser, and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch.
Bootstrap also detects four optional enhancements without blocking a healthy session: ast-grep 0.45.0 provides structural repository search under the collision-free `ast-grep` command, codeburn provides automatic task-usage snapshots, `@infisical/cli` provides Infisical credential workflows when a project needs that boundary, and Herdr enables later Herdr-backed dispatch when it is not already the resolved backend.
...
When Herdr is the resolved backend, the same missing executable becomes required and bootstrap offers the repository's pinned, checksum-verifying `bin/fm-install-herdr.sh` path after consent instead of deferring the failure until spawn.
...
FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=2000   # floor in milliseconds for how long the --claude turn-end guard polls for watcher health or a role-verified Stop auto-arm claim before deciding recovery ownership or failure progression; each poll costs the predicate on top of its sleep, and the auto-arm needs 1.1-1.7s of its own checks before it can claim
FM_CLAUDE_AUTOARM_EPOCH_FRESH=15   # seconds a recorded auto-arm outcome remains eligible for the current event epoch's recovery or failure decision; a rewake epoch naming the current session is instead honored regardless of age, because the recovery turn it created has no bounded length, and either way a rewake epoch is honored at most once
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh watcher before reporting FAILED, counted from the release of any recorded PR check migration holding state/.watch.lock rather than from the fork, with that extension capped at FM_GUARD_GRACE; an explicit value is exact, while the unset default is 10 scaled by load1/cores when the one-minute load average exceeds the online core count (rounded down, never above 45) and a flat 30 on Git Bash/MSYS; each state/.watch-cycle-exits.log row records the budget used as confirm_budget=, the load read at the attempt as load1=, and the started watcher's cumulative startup phases as startup=; the measured startup floor that budget must cover is about 3-5s to the first beacon on a 16-core host at load1 65-74 with 51 armed PR polls (main entry 0.5-1.5s, migration 1.1-2.2s, lock under 0.3s, first beacon 0.7-1.0s), because the migration certifies an unchanged check set by fingerprint and revalidates only records that moved; a set whose certification is absent or defective revalidates every record and was measured at 25-47s under that load, which is what the flat default cannot cover
FM_ARM_LOADAVG_PATH=/proc/loadavg   # file in /proc/loadavg format the load-scaled confirm default reads; unreadable keeps the flat default and records load1=none
FM_ARM_NPROC=   # online core count for the load-scaled confirm default; empty means nproc or getconf _NPROCESSORS_ONLN
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm is attached to an existing healthy watcher cycle
FM_OPENCODE_ARM_READY_TIMEOUT_MS=50000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; stays above the 45-second cap of the load-scaled confirm default, and defaults to 35000 on Windows to stay above the MSYS confirm budget
FM_PI_ARM_READY_TIMEOUT_MS=50000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached; stays above the 45-second cap of the load-scaled confirm default, and defaults to 35000 on Windows to stay above the MSYS confirm budget
...
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE; seconds a live watcher lock may have a stale beacon before re-arm errors, and how long a PR check migration or a starting watcher waits for a migration holding that lock
FM_SIGNAL_GRACE=30      # seconds to coalesce nearby status and turn-end signals into one wake
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; nonterminal progress verbs remain excluded even when their prose matches
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_CLASSIFY_CAPTAIN_HELD_VERB=captain-held   # leading status verb for an escalated decision parked awaiting firstmate or captain action; rendered into ship briefs and distinct from paused and blocked
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over, unless the task's latest working: line declares a long-run=<absolute path> artifact modified within FM_STALE_ESCALATE_SECS; inspection-only, never an automatic interrupt or restart
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared pause or captain-decision hold re-surfaces for a recheck in the watcher or away-mode daemon
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added
FM_CONTEXT_WARN_TOKENS=200000      # context size at which a live ordinary task's `context:` wake first fires, read from its own claude or codex session record by bin/fm-context-watch.mjs on each watcher poll; a task past this is probably too big to be one task
FM_CONTEXT_WARN_STEP=100000        # further context growth past the last surfaced level before the next `context:` wake; a compaction or relaunch resets the level, and each new compaction fires its own `context: <id> compacted (n=<count>)` wake
FM_CONTEXT_BIND_RETRY_SECS=60      # minimum seconds between attempts to bind a task whose stamped session record is not readable yet, so an unbound task never scans the session store every poll
