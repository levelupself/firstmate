...
The optional local `config/herdr-presentation-spaces` presence flag requests Herdr's default-off disposable single-task visual projection when no adopted cockpit frame owns placement; [Optional presentation spaces](herdr-backend.md#optional-presentation-spaces) owns its behavior, safety limits, recovery contract, and narrow locked session-start cleanup of exact restored idle-shell children.
...
The universal required toolchain is node, git, gh with GitHub auth via `gh auth login`, pnpm, ripgrep, pinned ShellCheck 0.11.0, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, and quota-axi v0.1.16 or newer.
