...
The aggregate job passes `--require-ci-lanes`, so a combined summary is refused unless its inputs are exactly the lane set `bin/fm-test-run.sh --list-ci-timing-artifacts` prints.
That inventory is derived from the lane inventory `bin/fm-test-run.sh` already owns, which keeps the required artifacts in step with the shard count rather than with a number written into the workflow.
