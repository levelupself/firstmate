...
Each poll-count knob of that budget is validated as a positive integer, and an unusable value falls back to its documented default with one warning naming the variable and the value, so a misconfigured knob is never reported as lock contention.
