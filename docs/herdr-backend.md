...
Each pane runs the tracked code root's `bin/fm-fleet-view.sh --watch --section <group>` with the operational home supplied separately through `FM_HOME`, so an isolated home never needs its own `bin/`; the command inherits the view's own priority ordering inside the pane, and the default groups preserve the order the single banner printed.
