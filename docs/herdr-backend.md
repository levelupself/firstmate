...
A validation failure names the exact check that failed - the pane unreachable, the server's answer untrusted, no fleet view running, a painter not bound to the pane's drawn size, a pane showing different sections than the frame recorded, or the pane moved out of the recorded tab - instead of one collapsed verdict, names the exact pane that failed, and is displayed without rebuilding or rewriting the frame.
