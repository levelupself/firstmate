...
The exact frame identity is recorded in that home's `state/.herdr-cockpit`, which binds the physical home, named session, workspace, tab, head pane, latest viewport pane, and every persistent live fleet pane together with the sections each one was launched to show.
...
The existing whole-fleet status view runs continuously in normal split panes recorded as part of the frame, so it survives client detach.
The region defaults to a full-width band above the supervisor holding roughly 28% of the frame, and `config/cockpit-layout` overrides that direction, order, and ratio ([`docs/configuration.md`](configuration.md) "Cockpit fleet layout").
That band is divided into one pane per section group rather than holding one all-sections banner, defaulting to decisions, then ready capacity, then running and blocked work, with `config/cockpit-sections` choosing how many panes there are and what each one holds ([`docs/configuration.md`](configuration.md) "Cockpit fleet sections").
Each pane runs the tracked code root's `bin/fm-fleet-view.sh --watch --section <group>` with the operational home supplied separately through `FM_HOME`, so an isolated home never needs its own `bin/`; the command groups live rows by project, preserves priority and state order within each project, and the default section groups preserve the order the single banner printed.
Building that region is the one automatic change to the operator's screen, so the command that applies it names the exact shape it is about to create - geometry and panes alike - and what it leaves untouched, before the first pane call, once for the whole region rather than once per pane.
An invalid layout or arrangement refuses and changes nothing rather than applying a shape nobody chose.
A region that cannot be finished is undone as a whole: every pane it added is removed and each removal verified, and where removal cannot be verified it reports NOT-RESTORED naming exactly the panes still on screen instead of claiming the screen was put back.
Nothing in that path closes, replaces, or re-splits a pane holding a live agent: each split only adds a pane, and a fleet-first order exchanges two panes' positions while Herdr keeps both pane ids, their tab and workspace, and any registered agent ([`docs/verification/cockpit-fleet-layout.md`](verification/cockpit-fleet-layout.md)).
The band is divided by splitting the newest fleet pane again, with creation-time proportional shares and optional per-pane weights governed by [`docs/configuration.md`](configuration.md) "Cockpit fleet sections"; counts changing during use never resize the region.
...
Each fleet painter is told which pane it is painting on its own command line, because the adapter created and recorded that pane and is the only party that knows.
The painter passes that same identity to its geometry probe, so both are bound to the exact recorded pane rather than to whatever pane identity the surrounding environment happens to carry ([`docs/verification/cockpit-fleet-layout.md`](verification/cockpit-fleet-layout.md) "A painter's ambient pane identity is not a guarantee").
...
If that authoritative read proves the exact recorded pane or its foreground cwd is gone, the bound painter reports the probe's exact reason and closes that exact pane once; an unreadable layout for a still-authoritative pane receives at most three consecutive reads before the same terminal eviction, and a successful read resets that budget.
The geometry probe also reports missing pane identity as a permanent cause, while the adapter's stated identity and frame validation prevent that ambient-identity failure in a current cockpit painter.
The painter captures probe diagnostics until a terminal path so they cannot corrupt a live frame, and a non-executable geometry command is refused when the banner starts.
...
It is a Firstmate workaround for the separately tracked upstream Herdr defect `herdr-pty-drawn-geometry-sync`, not the permanent source fix.
Building a region never closes the panes of a region built before it, so the frame record is also what retires their banners: a watched banner reads that record every redraw and stops painting as soon as the frame no longer names its pane for the sections it renders, leaving the pane itself untouched for the operator to keep or close.
That makes a rebuild self-cleaning rather than accumulating one live banner per generation, and it bounds the region to the panes the record actually names, so the newest board is the only board.
The same record admits one banner per bound pane: a second banner launched for a pane that already has a live one refuses and paints nothing rather than overwriting it, and a banner whose write fails stops instead of spinning against a terminal that has gone away.
Everywhere else the banner is unconstrained - the documented `bin/fm-fleet-view.sh --watch` fallback panel, a pane on another tab, a home with no adopted frame, and every non-Herdr runtime all keep painting exactly as before.
Banners started before this rule existed do not adopt it, because a running shell loop keeps the script it already parsed; closing their panes retires them, which is the same explicit pane close the frame uses everywhere else.
The panel renders its group's first section first and divides limited live-row capacity evenly among its projects, with compact project headers reporting their own width-clipped hidden counts so a noisy project cannot erase a quieter project's visible representation when one row per project fits.
Letting a data row or hidden-count summary wrap and scroll would cost the head, which is the part the ordering exists to protect.
...
`bin/fm-cockpit.sh zoom [on|off|toggle] [<fleet-pane>]` fills the frame with one fleet pane and restores it, which is the supported way to read it closely; the optional number selects a pane in the order `panel` lists them, and it changes no split, moves no pane, and binds to a key exactly like `next` and `prev`.
Frame validation proves that every recorded fleet pane still runs the watched fleet view for this home, is bound to its pane's drawn size, and still shows the sections the frame recorded for it, matching compatible operator-launched views by basename or absolute path and matching a painter's exact tracked executable plus that painter process's own live cwd against the operational home.
Home ownership is read from the painter's own entry in the pane's foreground process list, never from the pane-level foreground cwd: that field is one sample of whichever process is foreground at the instant of the read, and a watching painter forks short-lived children on every redraw, so it flaps to the code root while the painter has not moved and made a live region report dead at random.
The drawn-size binding is checked because a painter keeps running for as long as its pane does: without it a painter started before that binding existed satisfies every other check, is reported live indefinitely, and goes on sizing its frame to the wrong boundary.
The recorded pane identity is checked for the same reason and one more: it is what the drawn-size binding resolves a rectangle from, so a painter without it degrades on every redraw while every other check still passes, and it is matched against the exact recorded session and pane because a value any caller can spell proves nothing until it agrees with the frame.
A validation failure names the exact check that failed - the pane unreachable, the server's answer untrusted, no fleet view running, a painter not bound to the pane's drawn size, a painter that was not told which pane it is painting, a pane showing different sections than the frame recorded, or the pane moved out of the recorded tab - instead of one collapsed verdict, names the exact pane that failed, and is displayed without rebuilding or rewriting the frame.
One validated version 1 frame is upgraded in place by adding its fleet region and atomically publishing version 3, while an ambiguous legacy record remains untouched.
A version 2 record, written before the region could hold more than one pane, stays readable and live as the single-pane arrangement it describes.
...
`bin/fm-cockpit.sh panel` renders the same operator boundary as text: the native navigator role, exact pinned head, current viewport panes, every live fleet pane and what it shows, and `display=all-homes steer=current-home` boundary.
...
That reaction moves only panes this home's own task records claim, so the pinned head, every fleet pane, a pane with no agent, and another home's worker are all left alone even though the sidebar displays them.
...
`bin/fm-cockpit.sh show <task-id>` performs the same placement without any reaction running, and `next`/`prev` step the slot along this home's workers ordered by task id, wrapping at both ends and never targeting the pinned head or any fleet pane.
...
Those variables are evidence a launcher pane offers about itself, not an authority Firstmate can hand to a process it starts: a command run into a pane can set any of them to any value, so anything Firstmate needs a process to know for certain travels as an argument instead ([`docs/verification/cockpit-fleet-layout.md`](verification/cockpit-fleet-layout.md) "A painter's ambient pane identity is not a guarantee").
...
Task cleanup acquires that session lock before the task's isolated copy is returned, so a refusal to acquire it leaves the copy, every durable record, and the endpoint all intact for a plain rerun.
Because that lock is held across the whole destructive sequence, a healthy holder occupies it for tens of seconds, so cleanup queues behind it for the adapter-owned budget and refuses only once that budget is spent against a still-live holder; an abandoned hold is reclaimed immediately instead of waited on.
Concurrent cleanup of two tasks in one session therefore serializes rather than making the second one refuse.
That full budget applies only while the cleanup holds no session presentation lock yet; once one is held, every further session lock uses the adapter's short held bound instead, because a forced secondmate cleanup spans one session per child endpoint and concurrent multi-session cleanups have no common acquisition order.
Each poll-count knob of those bounds is validated as a positive integer once per process, and an unusable value falls back to its documented default with one warning naming the variable and the value, so a misconfigured knob is never reported as lock contention and never repeats per queued endpoint.
...
- Herdr 0.8.0 has pane zoom but no hover, floating pane, or overlay primitive, so a hover-to-expand fleet view is unavailable and is not emulated; the stacked region keeps fleet state visible and `fm-cockpit.sh zoom` is the supported way to look closer at one of its panes.
