...
| `fm-context-watch.sh`    | Print a live task's context size, peak, and compaction count from its own session record (`fm-context-watch.mjs` owns the watcher tick, the cache, and the `usage` token-attribution reader that `fm-effort-store.sh capture` persists) |
...
| `fm-model-bench-analyze.mjs` | Session-record arithmetic, independence byte-comparison, and table rendering behind `fm-model-bench.sh`, plus the shared per-harness record folds and tool class taxonomy that `fm-context-watch.mjs` reads |
...
| `fm-backlog-integrity.sh` | Guard, record, and reconcile task backlog lifecycle transitions                     |
| `fm-decision-hold.sh`    | Create, verify, complete, close, repair, and reconcile durable captain-held decisions |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, and recover a task whose endpoint is gone into its retained copy |
...
| `fm-project-git-setup.sh` | Apply shared project Git configuration during registration and refresh |
| `fm-merge-local.sh`      | Perform approved `local-only` landing with verified publication participants and visible read-only exclusions; the script header owns the contract |
...
| `fm-check-lib.sh`        | Validate custom-check registrations, prepare private execution snapshots, and certify the armed check set by fingerprint |
...
| `fm-pr-merge.sh`         | Merge a canonical GitHub PR, record provenance, and fast-forward its local mirror    |
