...
Follow the common wake-handling procedure emitted immediately above this fallback.
First cycle: drain queued wakes, then choose a supervision wait that the harness can actually wake from.
Ordinary wake: follow the common procedure, then repeat that verified wait while supervision is still required.
