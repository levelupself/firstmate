...
- OpenCode listens for `session.idle` in `.opencode/plugins/fm-primary-turnend-guard.js`, lets the watcher coordinator act first, and calls `client.session.promptAsync` once when the guard returns 2.
  OpenCode invokes the guard with `--opencode`, and the guard acknowledges the payload on stdout only after receiving and parsing it.
  A guard process that closes stdin or exits without that acknowledgement is treated as an indeterminate supervision check, not a successful allow: the plugin contains any stream error and forces the same bounded follow-up with an explicit input-delivery diagnostic.
...
The Claude mode allows the stop when the watcher is healthy, when `state/.claude-autoarm.lock` has a live `autoarm` role owner whose eventual failure must exit 2, or when `state/.claude-autoarm-epoch` records an actionable rewake this session owns.
Both Stop hooks start concurrently and neither one's exit decides the other's, so the guard cannot read its sibling's registration position as evidence; only these recorded proofs answer whether recovery is under way.
`state/.claude-autoarm-epoch` therefore records the session each outcome was handed to, and a `rewake` epoch naming the current session is that session's recovery turn: the previous cycle closed, the model handled the wake, and this Stop's auto-arm arms the next cycle.
The guard spends such a handoff exactly once per epoch in `state/.turnend-claude-rewake`, so one event epoch still yields exactly one recovery turn and an auto-arm that stops recovering blocks on the very next stop.
The handling turn it covers has no bounded length, so wall-clock age cannot decide whose recovery an epoch is; `FM_CLAUDE_AUTOARM_EPOCH_FRESH` (default 15 seconds) still admits an epoch written before the auto-arm recorded sessions, and still bounds the `failed` and `failed-suppressed` progression below.
That age arm spends the same session-keyed record, so an epoch honored inside the freshness window is not honored again once the window has passed, and a handoff this session cannot record as spent is not honored at all - including when `state/.turnend-claude-rewake` is not a plain file the guard can read back and rewrite.
Failing that, the guard polls for at least `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 2000 milliseconds, a floor because each poll costs the predicate on top of its sleep) for a claim, because the auto-arm needs 1.1-1.7 seconds of its own scope, ancestry, and need checks before it can claim and a shorter window cannot establish that it did not.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, a rewake handoff honored only for the session it names and spent exactly once, a refusal to honor one this session cannot record as spent, the routine cycle boundary and a genuinely unrecoverable absence both driven by the real Stop auto-arm, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, OpenCode acknowledged input delivery and closed-peer failure reporting, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
