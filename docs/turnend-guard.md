...
That age arm spends the same session-keyed record, so an epoch honored inside the freshness window is not honored again once the window has passed, and a handoff this session cannot record as spent is not honored at all.
