...
Both metadata fields must agree to identify the interactive session: `originator` must be `codex-tui` and `source` must be `cli`, so neither field alone can make a `codex exec` run that shares the tree look like the pane worker's rollout.
