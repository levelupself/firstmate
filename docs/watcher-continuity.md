...
Each record includes arm and watcher PIDs, start and end timestamps, exit code and signal, classified reason, beacon age, lock identity before and after close, the confirmation budget used with the one-minute load average read at the attempt, and successor disposition.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi's first-cycle-or-explicit-repair tool metadata and ownership-based redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers, gates successor establishment before firing the readiness deadline, matches the live successor PID through fallback and retirement, verifies single-flight behavior and OpenCode coordinator results for non-owner and owner lock states, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure.
The same suite covers ordinary same-process session replacement for `/new`, `/resume`, and `/fork`, same-instance shutdown-plus-start, stale prior-generation callbacks, repeated transitions with exactly one live cycle, disappearance of the shutting-down refusal after a valid replacement activates, and terminal quit still refusing late rearm.
Its positive fixture and driver checkpoints synchronize on signals rather than durations under the contract in [`tests/fm-checkpoint-helpers.sh`](../tests/fm-checkpoint-helpers.sh); duration sleeps remain only for negative assertions, while the intentionally retained process-death and post-driver teardown polls are documented at their sites.
That contract also owns why a checkpoint no surviving fixture can announce is reported by name rather than waited on, why fixture shutdown must finish before the checkpoint bus closes, and why one idle-time watchdog per driver remains a diagnostic backstop for a fixture that pins its driver instead of dying.
`tests/fm-checkpoint-helpers.test.sh` enforces those guarantees against real fixture processes, including one killed rather than exited, preserves the historical blocked-writer failure with a positive detector control, and pins the retry-gap case the liveness rule must not fire on.
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy, a loaded host that extends the default confirmation budget and records `confirm_budget=` and `load1=` in the lifecycle row through simulated `FM_ARM_LOADAVG_PATH` and `FM_ARM_NPROC` readers rather than real load, and that budget's floor, cap, exact explicit override, and unreadable-source fallback.
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, an arm that still confirms its watcher after a sibling PR check migration holds the lock past `FM_ARM_CONFIRM_TIMEOUT`, and a watcher that waits behind a recorded migration hold but stands down at once behind a recorded peer.
`tests/fm-watch-checkpoint.test.sh` sends TERM during a nested command substitution and proves the default signal disposition runs watcher cleanup without a Bash parser diagnostic, removes the owned lock, and publishes downtime.
`tests/fm-subagent-pretool-check.test.sh` proves Claude retains only the non-status Bash seatbelts.
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, exit-2 translation, and epochs that name the session their handoff was made to.
`FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` starts with the reproduced stale-lock state, runs session start first, completes two tokenless cycles, checks that the rewake epoch names the session id the real Stop payload carried, and checks the competing-live-owner negative control.
[`turnend-guard.md`](turnend-guard.md#regression-coverage) owns what `tests/fm-turnend-guard.test.sh` covers for the cooperative `--claude` guard.
