...
That contract also owns why a checkpoint no surviving fixture can announce is reported by name rather than waited on, and why one idle-time watchdog per driver covers the fixture that pins its driver instead of dying.
`tests/fm-checkpoint-helpers.test.sh` enforces both against real fixture processes, including one killed rather than exited, and pins the retry-gap case the liveness rule must not fire on.
