...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy, a loaded host that extends the default confirmation budget and records `confirm_budget=` and `load1=` in the lifecycle row through simulated `FM_ARM_LOADAVG_PATH` and `FM_ARM_NPROC` readers rather than real load, and that budget's floor, cap, exact explicit override, and unreadable-source fallback, plus the startup profile a started cycle's row carries, exercised through the same simulated stall rather than real load.
