...
It synchronizes on signals rather than on durations: its arm fixtures announce each point they reach and its drivers block on those announcements, under the contract in [`tests/fm-checkpoint-helpers.sh`](../tests/fm-checkpoint-helpers.sh), so the duration sleeps that remain are only the ones asserting that something never happens.
