...
`pane_cwd` follows a top-level shell `cd` but not the foreground subshell `treehouse enter` opens into the pane's copy (`treehouse get` opened the same kind of subshell before leases).
