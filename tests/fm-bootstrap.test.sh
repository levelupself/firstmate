...
SH
  ...
  all_out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  assert_contains "$all_out" "MISSING: shellcheck (required:" "the unsplit run lost its local diagnostic"
  ...
  assert_contains "$skip_out" "MISSING: shellcheck (required:" "the local half lost its own diagnostic"
  ...
  assert_not_contains "$only_out" "MISSING: shellcheck" "the network half repeated the local half's work"
