...
test_ship_mode_is_explicit_not_registry() {
  ...
  assert_grep "invoke /no-mistakes yourself" "$brief" \
...
test_terminal_report_cannot_be_written_at_commit() {
  ...
  for id_proj in "brief-term-nm:no-registry-proj" "brief-term-dp:direct-proj" "brief-term-lo:local-proj"; do
    case "${id_proj%%:*}" in
      brief-term-nm) mode=no-mistakes ;;
      brief-term-dp) mode=direct-PR ;;
      brief-term-lo) mode=local-only ;;
    esac
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "${id_proj%%:*}" "${id_proj##*:}" --mode "$mode" >/dev/null 2>&1
...
test_behavioral_tests_are_red_first() {
  ...
  for id_proj in "brief-red-nm:no-registry-proj" "brief-red-dp:direct-proj" "brief-red-lo:local-proj"; do
    id=${id_proj%%:*}
    case "$id" in
      brief-red-nm) mode=no-mistakes ;;
      brief-red-dp) mode=direct-PR ;;
      brief-red-lo) mode=local-only ;;
    esac
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" "${id_proj##*:}" --mode "$mode" >/dev/null 2>&1
    brief="$home/data/$id/brief.md"
...
test_reports_disclose_commit_and_proof_files() {
  ...
  for id_proj in \
    ...
    id=${id_proj%%:*}
    case "$id" in
      brief-proof-nm) mode=no-mistakes ;;
      brief-proof-dp) mode=direct-PR ;;
      brief-proof-lo) mode=local-only ;;
    esac
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" "${id_proj##*:}" --mode "$mode" >/dev/null 2>&1
    brief="$home/data/$id/brief.md"
