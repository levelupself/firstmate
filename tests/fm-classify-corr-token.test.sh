...
test_tokened_opener_opens_and_tokened_closer_closes() {
  ...
  # A blocked opener stays open through a captain-held park. Only an explicit
  # resolution closes a decision in this fork.
  ...
  case "$view" in
    *'task-blocked'*'[key=sortie-plan]'*) : ;;
    *) fail "a captain-held carrying a correlation token closed its key: $view" ;;
  esac
