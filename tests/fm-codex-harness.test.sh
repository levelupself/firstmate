...
make_codex_binding() {  # <case> <rollout-body> [<originator>] -> echoes <state-dir>
  local case_name=$1 body=$2 originator=${3:-codex-tui} root ws state day
  ...
  mkdir -p "$day" "$ws" "$state"
  {
    ...
    printf '{"session_id":"s-%s","cwd":"%s","originator":"%s","source":"cli","cli_version":"0.145.0"}}\n' \
      "$case_name" "$ws" "$originator"
    printf '%s' "$body"
