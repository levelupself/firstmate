...
make_fake_root() {
  ...
  # Teardown sources the shared return-prune and footprint helpers at startup.
  ln -s "$ROOT/bin/fm-build-output-lib.sh" "$fake/bin/fm-build-output-lib.sh"
  ln -s "$ROOT/bin/fm-pool-footprint-lib.sh" "$fake/bin/fm-pool-footprint-lib.sh"
  ...
  ln -s "$ROOT/bin/fm-backlog-integrity.sh" "$fake/bin/fm-backlog-integrity.sh"
  ln -s "$ROOT/bin/fm-backlog-tsv.sh" "$fake/bin/fm-backlog-tsv.sh"
  ...
  # Effort capture is a required teardown lifecycle producer. Stub its public
  # command here because this fixture intentionally owns no durable data store.
  cat > "$fake/bin/fm-effort-store.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fake/bin/fm-effort-store.sh"
...
META
  mkdir -p "$fake/data"
  cat > "$fake/data/backlog.md" <<BACKLOG
# Backlog

## In flight
- **$id** - Test fixture work (repo: fixture) (kind: ship)
