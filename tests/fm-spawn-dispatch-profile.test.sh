...
test_launch_capture_failure_prevents_spawn_success() {
  local rec id out status raw db
  id=profile-effort-capture-z20
  rec=$(make_spawn_case profile-effort-capture codex "$id")
  read_case_record "$rec"
  raw="$HOME_DIR/data/cost-attribution.tsv"
  mkdir "$raw"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 1 "$status" "spawn must fail when its launch record cannot become durable"
  assert_not_contains "$out" "spawned $id" "spawn reported success without a durable launch record"
  assert_contains "$out" "launch effort capture failed" "spawn did not identify the failed launch boundary"
...
const row = new DatabaseSync(process.argv[2], {readOnly: true})
  .prepare('SELECT started_at FROM task WHERE task_id = ?').get(process.argv[3])
if (typeof row?.started_at !== 'string' || !row.started_at) process.exit(1)
NODE
