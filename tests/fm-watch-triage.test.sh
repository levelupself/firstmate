...
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  ...
  await_watcher_loop "$state" "$!"
}

# Block until watcher <pid> is in its poll loop or has already exited, so the
# bounded waits a case runs afterwards measure poll cycles, not process startup.
# Startup - library sourcing, the PR-check migration scan, inactive-outcome
# reconciliation - costs a variable slice of a loaded host and has overrun the
# fixed 3s/4s budgets below on CI runners. The watcher claims state/.watch.lock
# with a fresh pid file naming its own pid during startup and touches the
# liveness beacon (state/.last-watcher-beat, fm-guard.sh's watcher-alive
# contract) at the top of every cycle, so a beat newer than this watcher's own
# lock pid file marks loop entry even when an earlier watcher's beat remains in
# the case. Bounded at 30s; a watcher that never gets there fails in the case's
# own assertions. Foreground sleeps only, so a caller's $! stays the watcher.
await_watcher_loop() {  # <state> <pid>
  local state=$1 pid=$2 lock_pid i=0
  lock_pid="$state/.watch.lock/pid"
  while [ "$i" -lt 300 ]; do
    if [ "$(cat "$lock_pid" 2>/dev/null || true)" = "$pid" ] \
      && [ "$state/.last-watcher-beat" -nt "$lock_pid" ]; then
      return 0
    fi
...
# How many stale wakes <state>'s durable queue holds for <window> - the count of
# supervision turns that window has cost. Prints 0 when no queue exists yet, so a
# fully absorbed window is asserted the same way as a noisy one.
count_window_stale_wakes() {  # <state> <window>
  local state=$1 window=$2
  [ -e "$state/.wake-queue" ] || { printf '0\n'; return; }
...
test_terminal_stale_surfaced() {
  ...
  pid=$!; await_watcher_loop "$state" "$pid"
...
# A still-live agent at an external-decision gate is the same declaration and gets
# the same bounded handling: the agent being up is what makes an idle park
# credible, not what invalidates it. The unchanged hash must not append the same
# wake on every watcher re-arm, and a residual wedge timer must be discarded.
test_declared_pause_is_bounded_whether_or_not_the_agent_exited() {
  ...
  # First sight of a fresh declaration is absorbed onto the pause cadence, not
  # spent as a supervision turn, even though the agent is still up.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live external-decision gate surfaced instead of using the pause cadence: $(cat "$out")"
  fi
  [ -e "$state/.paused-$key" ] || { reap "$pid"; fail "live external-decision gate did not record the pause cadence"; }
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional external-decision watcher stop"
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live external-decision gate escalated on the wedge timer: $(cat "$out")"
  fi
  ...
  [ "$wakes" -eq 0 ] || fail "live external-decision gate spent $wakes supervision turns inside its recheck window"
  pass "declared-pause and captain-held panes use the bounded pause cadence whether or not their agent exited"
}

# --- a LIVE worker parked on a captain decision -------------------------------
# The live 2026-08-08 case: a crewmate that appended `captain-held:` and went
# idle re-surfaced as a bare `stale: <window>` wedge again and again, because the
# bounded pause cadence could only engage once the backend reported the agent
# DEAD - and a worker parked on a decision is by definition still running. A
...
test_live_captain_held_park_stays_quiet_across_cycles() {
  ...
  printf 'needs-decision: pick a release route\ncaptain-held [key=route]: parked at the release-route decision\n' > "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-parked_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')

  # Four consecutive cycles, each with a different idle footer, so every one of
  # them reaches the stale path as a first sighting - the exact shape that made
  # the live pane re-surface. The agent stays up throughout (the pane's current
  # command is the recorded harness).
  round=1
  while [ "$round" -le 4 ]; do
    text="parked at the release-route gate - idle footer $round"
    printf '%s\n' "$text" > "$capture_file"
    printf '%s' "$(hash_text "$text")" > "$state/.hash-$key"
    printf '1\n' > "$state/.count-$key"
    PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
      FM_FAKE_TMUX_CURRENT_COMMAND=grok \
      FM_FAKE_CREW_STATE='state: unknown · source: pane · idle at the decision gate' \
    ...
    ack_stopped_cycle "$state" || fail "could not acknowledge live captain-held watcher cycle $round"
    round=$((round + 1))
  done

  wakes=$(count_window_stale_wakes "$state" "$window")
  [ "$wakes" -eq 0 ] || fail "a live captain-held park spent $wakes supervision turns across four cycles: $(cat "$out")"
  [ ! -s "$out" ] || fail "a live captain-held park printed a wake reason: $(cat "$out")"
  [ -e "$state/.paused-$key" ] || fail "a live captain-held park did not record the bounded pause cadence"
  [ ! -e "$state/.stale-since-$key" ] || fail "a live captain-held park started the wedge timer"
  pass "a live captain-held park is absorbed on the bounded cadence instead of re-surfacing every cycle"
}

# The companion bound: absorbing a live park must never be unbounded. Once the
# park's own status file has been quiet past PAUSE_RESURFACE_SECS it re-surfaces
# once, carrying the declared-park wording so a supervisor can tell a park from a
# wedge without opening the pane - then throttles again for the rest of that
# window, so a forgotten hold reaches a supervisor without nagging.
test_live_captain_held_park_resurfaces_on_the_long_cadence() {
  local dir state fakebin out capture_file statusf window key sig pid back text wakes
  dir=$(make_case live-captain-held-cadence); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; statusf="$state/parked.status"
  window="test:fm-parked"
  text='parked at the release-route gate'
  printf '%s\n' "$text" > "$capture_file"
  printf 'window=%s\nkind=ship\nharness=grok\nbackend=tmux\n' "$window" > "$state/parked.meta"
  printf 'captain-held [key=route]: parked at the release-route decision\n' > "$statusf"
  back=$(( $(date +%s) - 500 ))
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!; await_watcher_loop "$state" "$pid"
  wait_for_exit "$pid" 100 || fail "a long-quiet live captain-held park never re-surfaced"
  grep -F "stale: $window" "$out" >/dev/null || fail "the long-cadence recheck did not print a stale wake"
  grep -F "declared pause" "$out" >/dev/null \
    || fail "the long-cadence recheck used bare wedge wording instead of the declared-park wording: $(cat "$out")"
  grep -Fx "stale: $window" "$out" >/dev/null && fail "the long-cadence recheck surfaced the bare stale form"
  ...
  # cycle quiet, so the bound is once per window rather than once per poll.
  : > "$out"
  printf '%s' "$(hash_text "$text")" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
...
# --- declared long foreground job: artifact-backed relief from the turn-age bound
# A worker running a long foreground job whose output goes to a file (a full
# test suite, a long build) sits on a quiet busy pane for hours, so once its
# completed-turn age crosses BUSY_TURN_MAX_SECS the bound above escalates it
# every STALE_ESCALATE_SECS against healthy work. The worker can name the file
# its job keeps writing with a whole-word long-run=<path> token on its latest
# working: line; the bound then relaxes only while that file is actually still
# changing (modified within STALE_ESCALATE_SECS). The declaration alone proves
# nothing: a missing or quiet artifact escalates exactly as before, so a
# genuinely wedged job is detected no later than an undeclared one.

test_status_long_run_artifact_classifier() {
  local got
  got=$(status_long_run_artifact 'working: full suite under way, ~2h long-run=/tmp/suite.log') \
    || fail "long-run token on a working line not recognized"
  [ "$got" = /tmp/suite.log ] || fail "long-run artifact path not extracted: $got"
  ...
  status_long_run_artifact 'working: long-run=relative/path.log' \
    && fail "a relative artifact path was accepted"
  status_long_run_artifact 'paused: long-run=/tmp/suite.log' \
    && fail "a paused line carried a long-run declaration"
  status_long_run_artifact 'blocked: long-run=/tmp/suite.log hung' \
    && fail "a blocked line carried a long-run declaration"
  status_long_run_artifact 'working: no declaration here' \
    && fail "a plain working line yielded an artifact"
  status_long_run_artifact '' && fail "empty line yielded an artifact"
  status_is_captain_relevant 'working: full suite under way long-run=/tmp/suite.log' \
    && fail "a long-run working line became captain-relevant"
  pass "status_long_run_artifact: only a whole-word long-run=<absolute path> token on a working line declares an artifact"
}

# Common fixture: a pi busy pane whose completed-turn marker is already past the
# bound, with a wedge already mid-escalation so the test can see it collapse.
long_run_case() {  # <name> <id> <window> <status-line>  -> sets LR_DIR
  local name=$1 id=$2 window=$3 status_line=$4 dir state key sig
  dir=$(make_case "$name"); state="$dir/state"; LR_DIR=$dir
...
test_busy_pane_declared_long_run_with_live_artifact_collapses_wedge_ladder() {
  local dir state fakebin out window key artifact pid
  window="test:fm-busy-long-run-live"
  artifact="$TMP_ROOT/long-run-live.log"
  printf 'ok - 1\n' > "$artifact"
  long_run_case busy-long-run-live busy-long-run-live "$window" \
    "working: full suite under way, ~2h; long-run=$artifact"
  dir=$LR_DIR
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!; await_watcher_loop "$state" "$pid"
  # The stable-hash count advances at the START of each poll's scan of this pane,
  # so waiting for the second advance proves the first scan's triage completed and
  # the assertions below read a post-triage state, not a mid-poll one.
  if ! wait_count_at_least "$state/.count-$key" 3 150; then
    if kill -0 "$pid" 2>/dev/null; then
      reap "$pid"; fail "watcher never completed a poll of the busy long-run pane: $(cat "$out")"
    fi
    wait "$pid" 2>/dev/null || true
    fail "a busy pane with a live declared long-run artifact was escalated: $(cat "$out")"
  fi
  [ ! -s "$out" ] || fail "a busy pane with a live declared long-run artifact printed a wake reason"
  [ ! -e "$state/.stale-since-$key" ] || fail "a live declared long-run artifact did not clear the wedge timer"
  [ ! -e "$state/.wedge-escalations-$key" ] || fail "a live declared long-run artifact did not clear the escalation counter"
  [ "$(count_window_stale_wakes "$state" "$window")" = 0 ] || fail "a live declared long-run artifact still queued a stale wake"
  reap "$pid"
...
test_busy_pane_declared_long_run_with_quiet_artifact_still_escalates() {
  ...
  printf 'ok - 1\n' > "$artifact"
  # The artifact exists but stopped changing longer ago than the wedge threshold.
  set_mtime $(( $(date +%s) - 500 )) "$artifact"
  long_run_case busy-long-run-quiet busy-long-run-quiet "$window" \
    "working: full suite under way, ~2h; long-run=$artifact"
  dir=$LR_DIR
  state="$dir/state"; fakebin="$dir/fakebin"; out="$dir/watch.out"
  key=$(printf '%s' "$window" | tr ':/.' '___')
...
test_busy_pane_long_run_declaration_superseded_by_later_status_line() {
  local dir state fakebin out window artifact pid
  window="test:fm-busy-long-run-superseded"
  artifact="$TMP_ROOT/long-run-superseded.log"
  printf 'ok - 1\n' > "$artifact"
  long_run_case busy-long-run-superseded busy-long-run-superseded "$window" \
    "working: full suite under way; long-run=$artifact"
  dir=$LR_DIR
  state="$dir/state"; fakebin="$dir/fakebin"; out="$dir/watch.out"
  # A later working: note without the token is the current event: the earlier
  # declaration no longer describes what the worker is doing.
  printf 'working: suite finished, now fixing failures\n' >> "$state/busy-long-run-superseded.status"
  printf '%s' "$(seen_sig "$state/busy-long-run-superseded.status")" > "$state/.seen-busy-long-run-superseded_status"

  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
    FM_STATE_OVERRIDE="$state" FM_BUSY_TURN_MAX_SECS=1 FM_STALE_ESCALATE_SECS=240 FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!; await_watcher_loop "$state" "$pid"
  wait_for_exit "$pid" 100 || fail "a superseded long-run declaration kept suppressing the wedge escalation: $(cat "$out")"
  grep -F "possible wedge" "$out" >/dev/null || fail "superseded-declaration escalation did not flag a possible wedge"
  pass "only the latest status line can declare a long run: a later working: note without the token restores the ordinary bound"
}
